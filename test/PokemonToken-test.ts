...
describe("Pokemon", function () {
  ...
  describe("Minting", () => {
    ...
    it("Should return all owned ids of an account.", async () => {
      ...
      expect(await nft.ownedPokemonIds(acc1.address, 0)).to.eq(1);
      expect(await nft.ownedPokemonIds(acc1.address, 1)).to.eq(3);
      expect(await nft.ownedPokemonIds(acc2.address, 0)).to.eq(2);
      expect(await nft.ownedPokemonIds(acc2.address, 1)).to.eq(4);
    })
